    ...
    S: for<'r2, 'c2> Service<ServiceRequest<'r2, 'c2>, Response = Response, Error = Error> + Send + Sync,
{
    ...
    async fn call(&self, req: ServiceRequest<'r, 'c>) -> Result<Self::Response, Self::Error> {
        let ServiceRequest { req, client, address, request_timeout, response_timeout } = req;
        let mut count = 0;

        loop {
            let res = self.service.call(ServiceRequest { req, client, address, request_timeout, response_timeout }).await;
