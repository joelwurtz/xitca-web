...
use xitca_test::{Error, test_h1_server, test_h1_server_with_addr};
...
async fn h1_get_connection_closed_by_server() -> Result<(), Error> {
    ...
    let mut handle = test_h1_server_with_addr(fn_service(crate::handle), ip_port.to_socket_addrs().into_iter().next().unwrap().next())?;
    let mut res = c.get(&server_url).version(Version::HTTP_11).send().await?;
